...
import { Button, Card, CardMedia, Dialog, Divider, CircularProgress, FormControl, Grid, IconButton, InputLabel, MenuItem, Paper, Select, Stack, TextField, Typography } from '@mui/material';
...
const EditBlog = (props) => {
    ...
    const [_numberBasic, set_NumbeBasic] = useState('');
    ...
    const updateBlog = () => {
        ...
        // get key and body of text editors
        text_editors.map((value, index) => {
            bodies[index] = { key: value.key, body: value.value }
        })
        //get key & file id & file url from image editors
        let index = 0;
        ...
        axiosConfig.put(`/admin/blog/${props.id}`, blogObj)
            .then(res => {
                window.location.reload()
            })
            .catch(err => {
                setNotificationObj({
                    open: true, type: 'error', message: `Update blog has a problem!`
                })
                setTimeout(
                    () => setNotificationObj({
                        open: false,
    ...
    useEffect(() => {
        ...
        if (imageFileBasic && formData) {
            axiosConfig.post('/admin/uploader', formData)
                .then(res => {
                    valueArr.push({ 'src': res.data.files[0].image_url, 'id': res.data.files[0].file_id })
                    result = valueArr;
                    setMainImage([...result]);
    ...
    const getBlog = (blogId) => {
        axiosConfig.get('/admin/blog/all?title=&page=1&limit=20')
            .then(res => {
                let blogs = res.data.blogs.filter(b => b.attributes.length != 0 && ((b.attributes[0].value).includes(searchValue)));
                setBlogs(res.data.blogs.filter(b => b.attributes.length != 0 && ((b.attributes[0].value).includes(searchValue))))
                axiosConfig.get(`/admin/blog/${blogId}`)
                    .then(async res => {
                        let title = res.data.blog_attributes.filter(attribute => attribute.attribute_id == 1)[0].value;
                        title = toCamlize(title)
                        setTitle(title);
                        ...
                        //get main image
                        let valueArr = [];
                        valueArr.push({ 'src': res.data.file_url.image_url, 'id': res.data.file_url.id })
                        setMainImage([...valueArr])
                        ...
                        res.data.blog_images.map(img => {
                            ...
                            if (key !== '' && key == img.key) {
                                image.push({ src: img.file_url, id: img.file_id })
                                image[1].id = img.file_id;
                                image[1].src = img.file_url;
                                editorOrImageValues[img.key] = { key: img.key, value: image, isTextOrImage: 'Image' }
                                key = '';
                                image = [{ src: '', id: '' }]
                            } else {
                                image[0].id = img.file_id;
                                image[0].src = img.file_url;
                                editorOrImageValues[img.key] = { key: img.key, value: image, isTextOrImage: 'Image' }
                                key = img.key
                            }
                            key1 = img.key;
                        ...
                        let relatedPosts = res.data.blog_attributes.filter(attribute => attribute.attribute_id == 3)
                        relatedPosts.map((post, index) => {
                            blogs.map(blog1 => {
                                if (blog1.id == post.value.slice(-2)) {
                                    dropdownSelectedValue[index] = { nthBlig: index, idBlog: blog1.id };
                                    postLink[index] = post.value;
                                    axiosConfig.get(`/admin/blog/${blog1.id}`).then(res => {
                                        selectedPosts[index] = {
                        ...
                        let relatedProducts = res.data.blog_attributes.filter(attribute => attribute.attribute_id == 4)
                        await axiosConfig.get(`/admin/product/all?limit=1000&page=1&language_id=1&status=1&category_id&name=`
                            ...
                            relatedProducts.map((product, index) => {
                                products.map(p => {
                                    if (p.id == product.value.split(':')[1]) {
                                        dropdownSelectedProductValue[index] = { nthProduct: index, idProduct: p.id };
                                        productLink[index] = product.value;
                        ...
                        //get related posts values and id 
                        setDropdownSelectedValue(dropdownSelectedValue);
                        setDropdownSelectedProductValue(dropdownSelectedProductValue)
                        setKey(key1 + 1);
                ...
                <Grid md={12} >
                    <FormControl fullWidth color='P'>
                        ...
                        <Select
                            ...
                            value={dropdownSelectedValue[nthBlog] != undefined ? (dropdownSelectedValue[nthBlog].idBlog !== '' ? dropdownSelectedValue[nthBlog].idBlog : '') : ''}
                            label={title + " "}
                            onChange={(e) => {
                ...
                <Grid md={12} >
                    <FormControl fullWidth color='P'>
                        ...
                        <Select
                            labelId="demo-simple-select-label"
                            id="demo-simple-select"
                            value={dropdownSelectedProductValue[nthProduct] != undefined ? (dropdownSelectedProductValue[nthProduct].idProduct !== '' ? dropdownSelectedProductValue[nthProduct].idProduct : '') : ''}
                            label={title + " "}
                            onChange={(e) => {
                                ...
                                products.map(p => {
                                    if (p.id == e.target.value) {
                                        ...
                                        selectedProducts[nthProduct] = { product: p, image_url: p.file_urls[0], name: p.name }
                                    }
                                })
                                setClicked(clicked + 1)
                            }}
                        >
                            ...
                            {products.map((p) => {
                                let result = false;
                                dropdownSelectedProductValue.map(selectedProductValue => {
                                    if (selectedProductValue.idProduct === p.id) {
                                        result = true;
                                    }
                                })
                                return (
    ...
    return (
        <Grid breadcrumb={bread} pageName='Edit Blog'>
            <Grid container xs={12} display='flex' spacing={2}>
                <Grid item xs={9}>
                    <Paper sx={{ width: "100%", minHeight: '300px' }} >
                        ...
                        <Grid xs={12}>
                            <Grid p={2}>
                                <TextField id="outlined-basic" value={!/\S/.test(title) || title == null ? "Title is empty." : title} onChange={(e) => setTitle(toCamlize(e.target.value))} color='P' fullWidth label="Title" variant="outlined" />
                            </Grid>
                                                ...
                                                >
                                                    ...
                                                    >
                                                        ...
                                                        <Grid item mt={0.3} ml={0.3} display='flex' flexDirection='row-reverse'>
                                                            <IconButton
                                                                sx={{ color: 'Black.main', borderRadius: 30, mt: 2, mr: 2 }}
                                                                aria-label="delete"
                                                                onClick={() => {
                                                                    setClicked(clicked + 1);
                            ...
                            <Grid p={2}>
                                <TextField id="outlined-basic" value={!/\S/.test(enDescription) || enDescription == null ? "Description is empty." : enDescription} onChange={(e) => setEnDescription(e.target.value)} color='P' fullWidth label="English Description"
                                                            ...
                                                            <Grid p={2} display="flex" xs={editorOrImageValue.value.length == 1 ? 12 : (window.innerWidth > 950 ? 6 : 12)} >
                                                                ...
                                                                >
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <Grid item mt={0.3} ml={0.3} display='flex' flexDirection='row-reverse'>
                                                                            <IconButton
                                                                                sx={{ color: 'Black.main', borderRadius: 30, mb: 2, mr: 1 }}
                                                                                aria-label="delete"
                                                                                onClick={() => {
                                        ...
                                        <Grid ml={2} mr={2} mt={2}>
                                            <CKEditor
                                                ...
                                                onChange={(event, editor) => {
                                                    // how to know if event was fired via user initiated event or data being loaded in.
                                        ...
                                        if (blog.id === post.blog.id) {
                                            return (
                                                <Grid xs={12} mb={5}>
                                                    ...
                                                    <Grid xs={7} mb={1}><Typography variant='h10' pl={4}>{post.en_description !== "null" ? post.en_description : "Description is empty."}</Typography></Grid>
