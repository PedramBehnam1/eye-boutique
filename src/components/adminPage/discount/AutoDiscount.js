...
const AutoDiscount = () => {
  ...
  useEffect(async ()=>{
    let discounts =''
    await axiosConfig.get("/admin/discount/all").then((res) => {
  ...
  const generateCode = ()=>{
    ...
    const _randomNumber = parseInt((Math.random()*(10-(-1)))+(-1))
    const _randomNumber_ = parseInt((Math.random()*(10-(-1)))+(-1))
    
    const randomCharacter = englishAlphabet[parseInt((Math.random()*(25-(-1)))+(-1))]
    const _randomCharacter = englishAlphabet[parseInt((Math.random()*(25-(-1)))+(-1))]
    const _randomCharacter_ = englishAlphabet[parseInt((Math.random()*(25-(-1)))+(-1))]
    let code = randomCharacter+''+_randomCharacter+''+_randomCharacter_+'-'+randomNumber+''+_randomNumber+''+_randomNumber_;
    return code;
  }
  ...
  useEffect(() => {
    ...
    if (CheckApplies == "Specific products" && selectedSpecificProducts.length != 0) {
      setSelectedProducts(selectedSpecificProducts.filter(p => p.id != 109))
    }
  ...
  const handleChangeProductY = (e, product) => {
  ...
  const deleteProducts = (type, selectedRow) => {
    ...
    if (type == "buys" || type == "Apllies") {
      ...
      if (CheckAppliesGets == "Specific collections") {
        let productsGet = filterVluesGets.filter(product => selectedRow.id != product.id);
        productsGet.map(p => {
          if (p.category_id == selectedRow.category_id) {
            result = true
          }
        ...
        setSelectedCollectionsProductsGets(productsGet);
        setFilterValusGets(productsGet);
        if (result == false) {
        ...
        let productsGet = filterVluesGets.filter(product => selectedRow.id != product.id);
        setFilterValusGets(productsGet);
        setSelectedSpecificProductsGets(productsGet)
        setSelectedProductsIdsGets(selectedProductsIdsGets.filter(id => id != selectedRow.id))

      } else if (CheckAppliesGets == "Customer List Y") {
        let productsGet = filterVluesGets.filter(user => selectedRow.id != user.id);
        setFilterValusGets(productsGet);
        setSelectedCustomersUsersGets(productsGet)
  ...
  return (
    <AdminLayout breadcrumb={bread} pageName="Add Auto Discount">
      <Grid display={"flex"}>
        <Grid xs={9} pr={2}>
          <Paper sx={{ width: "100%", minHeight: "300px" }}>
            ...
            >
              <Grid xs={12} p={3} textAlign="start">
                <Typography variant="h3">Auto Discount</Typography>
              </Grid>
            </Grid>
                                            ...
                                            <FormControlLabel
                                              control={
                                                <Checkbox
                                                  ...
                                                  onChange={(e) => (CheckAppliesGets != "Customer List Y" ? handleChangeProductY(e, selectedRow) : handleChangeUser(e, selectedRow))}
